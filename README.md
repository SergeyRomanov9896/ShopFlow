...
- `ruff` и `mypy` для проверки качества кода;
...
3) Создание категории и добавление товара:
```python
...
4) Попытка добавить в категорию значение неверного типа:
```python
category.add_product("не товар")
# Товар не является экземпляром класса Product
```

5) Покрытие блока запуска как скрипта:
```bash
python -m src.shopflow.shop
